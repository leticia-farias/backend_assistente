...
Router packageRoutes(GeminiService geminiService, PackageService packageService) {
  ...
  // Rota corrigida: de '/ping' para '/' (dentro do grupo /packages)
  router.get('/ping', (req) => Response.ok(jsonEncode({'status': 'ok', 'message': 'pong'}),
      headers: {'content-type': 'application/json'}));

  // Rota corrigida: de '/packages/all' para '/all'
  router.get('/all', (req) async {
  ...
  // Rota corrigida: de '/packages/suggest' para '/suggest'
  router.post('/suggest', (req) async {
    try {
      ...
      print('Erro interno no endpoint /suggest: $e');
